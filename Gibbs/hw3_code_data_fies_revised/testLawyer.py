...
#Creating random mask for test edges versus training edges in the graph
for ii in range(N):
  for jj in range(N):
    if ii != jj:
...
#Training data
graphMask_trn = graphMask*(graphMask > 0)
...
for qq in range(numTrial):
  z,pi,W,lp,_ = mGibbs(graphMask_trn, K, alphaSB, numIter)
  logProb.append(lp)
